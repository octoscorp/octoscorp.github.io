...
window.addEventListener("load", (load_event) => {
    document.querySelectorAll(".post-preview-card").forEach((card) => {
        ...
        let gen_url = create_image(post_id, 40, 10, pixel_callback = pixel_thresholding);
        card.querySelector(".post-preview-bg").style.backgroundImage = `url(/assets/images/${post_id}.png), url(${gen_url}), url(/assets/images/post-preview-default.png)`;
    });
});

function pixel_thresholding(channel_values) {
    let avg_brightness = (channel_values[0] + channel_values[1] + channel_values[2]) / 3;
